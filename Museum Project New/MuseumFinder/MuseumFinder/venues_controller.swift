    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if(searchActive){
            ...
            if let musuemData=NSData(contentsOfURL:NSURL(string: newurlPath)!){
                ...
                do { let musuemInfo: AnyObject! = try NSJSONSerialization.JSONObjectWithData(musuemData, options: NSJSONReadingOptions(rawValue: 0))
                    if let info = musuemInfo as? Array<NSDictionary> {
                        if info.count >= 1{
                            
                            let myEntry : AnyObject! = info[0]
                            _ = myEntry["commonname"] as! String
                            
                            _  = CLLocation(latitude: centerCoordinate.latitude, longitude: centerCoordinate.longitude)
