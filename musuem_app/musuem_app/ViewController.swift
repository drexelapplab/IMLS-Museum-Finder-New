...
//    let queue:NSOperationQueue = NSOperationQueue()
    ...
    func findByAddress(enteredLoc : String){
        ...
        geocoder.geocodeAddressString(enteredLoc, completionHandler: {(placemarks: [AnyObject]!, error: NSError!) -> Void in
            if(error != nil) {
...
//        }
    ...
    func updateMusuems(loc: CLLocation){
        ...
        if let musuemData=NSData(contentsOfURL:url!){
            if let positions: AnyObject! = NSJSONSerialization.JSONObjectWithData(musuemData, options: NSJSONReadingOptions(0), error: nil){
...
//        }.resume()
    ...
    func mapView(mapView: MKMapView!, viewForAnnotation annotation: MKAnnotation!) -> MKAnnotationView! {
        ...
        if anView == nil {
            ...
            var directionsButton: UIButton = UIButton.buttonWithType(UIButtonType.System) as! UIButton
...
//            directionsButton.setBackgroundImage(UIImage(named: "bentArrow.jpg") as UIImage?, forState: .Normal)
    ...
    func mapView(mapView: MKMapView!, annotationView view: MKAnnotationView!, calloutAccessoryControlTapped control: UIControl!) {
        ...
        var extString = NSURL(fileURLWithPath: view.annotation.title!)!.relativeString!.stringByReplacingOccurrencesOfString("&", withString: "%26").stringByReplacingOccurrencesOfString("+", withString: "%2B")
        ...
        }else if control == view.leftCalloutAccessoryView
        {
            ...
            var urlPath = "https://data.imls.gov/resource/bqh6-bapa.json?commonname=\(extString)&$select=location,commonname"
            let url = NSURL(string: urlPath)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if let ext = NSURL(fileURLWithPath: (tableView.cellForRowAtIndexPath(indexPath)?.textLabel?.text)!)
        {
